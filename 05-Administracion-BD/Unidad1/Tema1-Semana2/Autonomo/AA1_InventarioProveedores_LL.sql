...
CREATE DATABASE InventarioProveedores_LL;

USE InventarioProveedores_LL;
GO

-- ----------------------------------------------------------
-- Tabla: Proveedores
-- ----------------------------------------------------------
-- OBJECT_ID('nombre_tabla', 'U') verifica si la tabla existe (U = User Table)
...
BEGIN TRY
    BEGIN TRANSACTION;
        ...
        -- Declaración de variable para almacenar el stock después de la compra
        DECLARE @stock_post_compra INT; 
        -- Asignación del valor del stock actualizado a la variable
        SELECT @stock_post_compra = stock FROM Productos WHERE producto_id = 1;
        -- Imprime el stock actualizado después de la compra 
